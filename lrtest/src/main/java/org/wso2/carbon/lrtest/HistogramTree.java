...
public class HistogramTree implements Serializable {
	...
	public Map<Integer, Integer[]> groupBins(int noofBins) {
		...
		Logger.getRootLogger().debug(
				"Total number of Data Points -> " + this.subTotal);
	...
	private Map<Integer, Integer[]> groupBins(
		...
		// This variable will combine the child nodes that will send to the next
		// level
		...
		for (int i = 0; i < dimensionSize; i++) {
			...
			for (HistogramTree node : subtree.values()) {
				if (node.getCordinate()[level] == i) {
					...
					// if we are in the last level add current nodes else add
					// children
			...
			if ((splitTotal > qty2split)
					|| (((splitTotal <= qty2split) && (qty2split < (splitTotal + nextLoopTotal))) && ((qty2split - splitTotal) < (splitTotal
							+ nextLoopTotal - qty2split)))) {
				// Make a comparison of for next start value
				if (level == (histogramHelper.getNoofDimensions() - 1)) {
					binGroup.putAll(createGroup(nextTree));
				} else {
					binGroup.putAll(groupBins(nextTree, level + 1, qty2split,
							divideFactor));
				}
