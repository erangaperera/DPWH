...
public class HistogramHelper implements Serializable, IHistogramHelper {
	 ...
	 */
	public int addDimension(int featureColumn, double minValue,
			double maxValue, int splits) {
	 ...
	 */
	public int addDimensions(int[] featureColumns, double[] minValues,
			double[] maxValues, int[] splits) {
		for (int i = 0; i < featureColumns.length; i++) {
			addDimension(featureColumns[i], minValues[i], maxValues[i],
					splits[i]);
		}
		return this.dimensions.size();
	}
	...
	private int getFeatureSplitNo(int dimension, double featureValue) {
		return (int) ((featureValue - minValues.get(dimension))
				* splitSizes.get(dimension) / (maxValues.get(dimension) - minValues
				.get(dimension)));
	}
